...
def set_julia(julia_exec_path: str):
    ...
    """
    return Julia(runtime=julia_exec_path)
...
def pre_processing(
    ...
    if threshold != 0.06:
        msg = (
            "WARNING: The default threshold (0.05) dropped too many points"
            " and indicated a data-quality concern. Please be careful when"
            " using the filtered data. Current threshold is {threshold-0.01}"
        )
        print(msg)
...
def save_processed_data(input_name: str, data_stored: "pd.DataFrame") -> str:
    """
...
def EIS_auto(
    ...
    path_data_preprocessed = save_processed_data(
        input_name=data_path, data_stored=data_processed
    )
...
if __name__ == "__main__":
    ...
    # Analysis board
    results = EIS_auto(
        impedance=measurements, freq=frequencies, data_path=data_path, iter_number=100
    )
    print(results)
